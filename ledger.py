...
class PersonalFinanceLedger:
    ...
    def _load_transactions(self) -> None:
        ...
        try:
            ...
            console.print(f"[green]Data loaded successfully from {self.data_file}.[/green]")
            ...
            console.print("[bold red]Error loading data: JSON file is corrupted or empty. Starting new ledger.[/bold red]")
            self.transactions = []
    ...
    def add_transaction(self, tx_type: TransactionType, category: str, amount: float, description: str) -> None:
    ...
    def export_data(self, filename: str) -> None:
        ...
        try:
            with open(filename, 'w') as f:
                json.dump(self.transactions, f, indent=4)
            console.print(f"[bold cyan]📤 Data successfully exported to {filename}[/bold cyan]")
        except IOError:
            console.print("[bold red]Error: Could not write to the specified file.[/bold red]")
    ...
    def import_data(self, filename: str) -> None:
        ...
        try:
            ...
            if not isinstance(imported_data, list) or not all(isinstance(d, dict) for d in imported_data):
                raise ValueError("Imported file format is incorrect.")

            self.transactions = imported_data
            self._next_id = max((t['id'] for t in self.transactions), default=0) + 1
            self._save_transactions()
            console.print(f"[bold green]📥 Data successfully imported from {filename}.[/bold green]")
        except FileNotFoundError:
            console.print(f"[bold red]Error: File not found at {filename}.[/bold red]")
        except (json.JSONDecodeError, ValueError) as e:
            console.print(f"[bold red]Error importing data: {e}. File format may be invalid.[/bold red]")

# --- 3. CLI Interaction Functions ---

def display_transactions(transactions: List[Transaction], title: str = "Transaction History") -> None:
...
def handle_add(ledger: PersonalFinanceLedger) -> None:
    ...
    tx_type: str = Prompt.ask("Is this [bold blue]Income[/bold blue] or [bold blue]Expense[/bold blue]?",
                              choices=["income", "expense"], default="expense")
    category = Prompt.ask("Enter Category (e.g., [green]Salary[/green], [red]Groceries[/red])")
    ...
    if amount <= 0:
        console.print("[bold red]Amount must be greater than zero. Transaction cancelled.[/bold red]")
        return
...
def handle_view(ledger: PersonalFinanceLedger) -> None:
    ...
    filter_choice = Prompt.ask("Filter by [bold]type[/bold] (income/expense) or [bold]category[/bold]? (type 'all' for everything)",
                               default="all")
    ...
    if filter_choice.lower() == 'all':
        ...
        filtered = [t for t in ledger.transactions if t['type'] == filter_choice.lower()]
...
def handle_summary(ledger: PersonalFinanceLedger) -> None:
    ...
    table.add_row("[green]Total Income[/green]", f"[green]${summary['income']:.2f}[/green]")
    table.add_row("[red]Total Expense[/red]", f"[red]${summary['expense']:.2f}[/red]")

    balance_style = "bold green" if balance >= 0 else "bold red"
    table.add_row(f"[{balance_style}]Current Balance", f"[{balance_style}]${balance:.2f}[/{balance_style}]")
...
def handle_delete(ledger: PersonalFinanceLedger) -> None:
    ...
    tx_id = IntPrompt.ask("Enter the [bold red]ID[/bold red] of the transaction to delete")
...
def run_cli() -> None:
    ...
    while True:
        ...
        user_input: str = Prompt.ask("[bold green]Ledger[/bold green] >").lower().strip()
        ...
        if user_input in ("exit", "quit"):
            ...
            console.print(f"[bold red]Unknown command: '{user_input}'. Type 'help' for a list of commands.[/bold red]")
